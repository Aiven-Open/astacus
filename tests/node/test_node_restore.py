...
def test_restore(snapshotter, storage, tmpdir):
    ...
    restorer.restore_from_storage(storage=storage, progress=Progress(), snapshotstate=ss1)
    ...
    relpath2snapshotfile = {snapshotfile.relative_path: snapshotfile for snapshotfile in ss1.files}
    for snapshotfile in ss2.files:
        snapshotfile.mtime_ns = relpath2snapshotfile[snapshotfile.relative_path].mtime_ns
    assert ss1 == ss2
