...
class CassandraTestEnv(CassandraTestConfig):
    ...
    def __init__(self, *, app, client, mocker, tmpdir: py.path.local):
...
def test_api_cassandra_subop(app: FastAPI, ctenv: CassandraTestEnv, mocker: MockerFixture, subop: ipc.CassandraSubOp):
...
class TestCassandraRestoreSSTables:
    ...
    def fixture_make_sstables_request(self, astacus_node_cassandra) -> Callable[..., ipc.CassandraRestoreSSTablesRequest]:
...
def test_dangerous_ops_not_allowed_on_read_access_level(ctenv: CassandraTestEnv, dangerous_op: ipc.CassandraSubOp):
    ctenv.lock()
    ctenv.setup_cassandra_node_config()
