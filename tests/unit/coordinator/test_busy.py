...
def test_not_busy_if_no_coordinator_state(app: Starlette, client: TestClient) -> None:
...
def test_not_busy_if_failed_or_done(app: Starlette, client: TestClient, finished_status: Op.Status) -> None:
...
def test_busy_if_starting_or_running(app: Starlette, client: TestClient, finished_status: Op.Status) -> None:
    stats = StatsClient(config=None)
    operation = DummyOp(info=Op.Info(op_id=1, op_name="DummyOp", op_status=finished_status), op_id=1, stats=stats)
    app.state.coordinator_state.op = operation
    app.state.coordinator_state.op_info = operation.info
