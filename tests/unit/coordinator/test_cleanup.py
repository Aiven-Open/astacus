...
def _run(
    *,
    client: TestClient,
    populated_storage: StorageManager,
    app: FastAPI,
    ...
    assert len(populated_storage.get_json_store("x").list_jsons()) == 2
    populated_storage.get_hexdigest_store("x").upload_hexdigest_bytes("TOBEDELETED", b"x")
    assert len(populated_storage.get_hexdigest_store("x").list_hexdigests()) == 2
    ...
    assert len(populated_storage.get_json_store("x").list_jsons()) == exp_jsons
    assert len(populated_storage.get_hexdigest_store("x").list_hexdigests()) == exp_digests


@pytest.mark.parametrize("fail_at", FAILS)
def test_api_cleanup_flow(fail_at: int | None, client: TestClient, populated_storage: StorageManager, app: FastAPI) -> None:
    _run(
        fail_at=fail_at,
        client=client,
        populated_storage=populated_storage,
        app=app,
...
def test_api_cleanup_retention(
    data: tuple[ipc.Retention, int, int], client: TestClient, populated_storage: StorageManager, app: FastAPI
) -> None:
