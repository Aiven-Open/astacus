...
from astacus.common.utils import path_matches_glob
...
class Snapshotter(ABC, Generic[T]):
    ...
    def _embedded_file_size_max_for_file(self, file: SnapshotFile) -> int | None:
        ...
        for group in self._groups:
            if self._matches_group(file.relative_path, group):
                groups.append(group)
        assert groups
        head, *tail = groups
        for group in tail:
    ...
    def _matches_group(self, path: Path, group: SnapshotGroup) -> bool:
        return path_matches_glob(path, group.root_glob) and not any(path.name == e for e in group.excluded_names)
