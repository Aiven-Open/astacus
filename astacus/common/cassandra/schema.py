...
class CassandraNamed(AstacusModel):
    ...
    @classmethod
    def from_cassandra_metadata(cls, metadata):
        return cls(name=metadata.name, cql_create_self=metadata.as_cql_query(formatted=False))

    def restore_if_needed(self, cas: CassandraClient, existing_map: dict):
...
class CassandraUserType(CassandraNamed):
    ...
    def from_cassandra_metadata(cls, metadata: cm.UserType) -> 'CassandraUserType':
        return cls(
            name=metadata.name, cql_create_self=metadata.as_cql_query(formatted=False), field_types=metadata.field_types
        )

    def restore_in_keyspace(self, cas: CassandraClient, keyspace_metadata: cm.KeyspaceMetadata):
...
class CassandraFunction(CassandraNamed):
    ...
    def get_metadata_dict(self, keyspace_metadata: cm.KeyspaceMetadata):
...
class CassandraTable(CassandraNamed):
    ...
    def from_cassandra_metadata(cls, metadata: cm.TableMetadata) -> 'CassandraTable':
        return cls(
            # CassandraNamed
            name=metadata.name,
            cql_create_self=metadata.as_cql_query(formatted=False),
    ...
    def restore_post_data_in_keyspace(self, cas: CassandraClient, keyspace_metadata: cm.KeyspaceMetadata):
...
class CassandraKeyspace(CassandraNamed):
    ...
    def iterate_user_types_in_restore_order(self) -> Generator[CassandraUserType, None, None]:
        pending = {x.name: x for x in self.user_types}
        while pending:
            changed = False
            for user_type_name, user_type in list(pending.items()):
                if any(field_type_name in pending for field_type_name in user_type.field_types):
                    continue
                yield user_type
    ...
    def restore(self, cas: CassandraClient):
        metadata = cas.cluster.metadata
    ...
    def restore_post_data(self, cas: CassandraClient):
        metadata = cas.cluster.metadata
        keyspace_metadata: cm.KeyspaceMetadata = metadata.keyspaces[self.name]
...
class CassandraSchema(AstacusModel):
    ...
    @classmethod
    def from_cassandra_client(cls, cas: CassandraClient) -> 'CassandraSchema':
        """Retrieve the schema using CassandraClient"""
        ...
        cas.cluster.refresh_schema_metadata(max_schema_agreement_wait=10.0)

        if not cas.cluster.metadata.keyspaces:
            # there should always be system keyspaces, even when there are no user ones
            raise ValueError("No keyspaces in schema metadata")
        return cls(
            keyspaces=sorted(
                CassandraKeyspace.from_cassandra_metadata(metadata) for metadata in cas.cluster.metadata.keyspaces.values()
            )
        )
    ...
    def restore_pre_data(self, cas: CassandraClient):
        ...
        for keyspace in self.keyspaces:
            # Omitted for now, assume the system was configured with
            # proper replication
