...
class M3DBManifest(msgspec.Struct, kw_only=True, frozen=True):
    etcd: ETCDDump
    placement_nodes: Sequence[m3placement.M3PlacementNode]


class M3DBPlugin(CoordinatorPlugin, frozen=True):
    etcd_url: str
    environment: str
    placement_nodes: Sequence[m3placement.M3PlacementNode]
...
class PrepareM3ManifestStep(Step[Dict[str, Any]]):
    ...
    async def run_step(self, cluster: Cluster, context: StepsContext) -> Dict[str, Any]:
        ...
        return msgspec.to_builtins(M3DBManifest(etcd=etcd, placement_nodes=self.placement_nodes))
...
class RewriteEtcdStep(Step[Optional[ETCDDump]]):
    ...
    async def run_step(self, cluster: Cluster, context: StepsContext) -> Optional[ETCDDump]:
        ...
        plugin_manifest = msgspec.convert(backup_manifest.plugin_data, M3DBManifest)
        etcd = copy.deepcopy(plugin_manifest.etcd)
