...
async def _list_backups(
    *, storage: Annotated[str, Body()] = "", c: Coordinator = Depends(), request: Request
    ...
    try:
        cache = get_cached_entries(cached_list_response.list_response) if cached_list_response is not None else {}
...
def get_cached_entries(list_response: ipc.ListResponse) -> CachedListEntries:
    return {
