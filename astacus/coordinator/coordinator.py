...
class Coordinator(op.OpMixin):
    ...
    def create_op(self, op: type[CoordinatorOpT], *args: Any, **kwargs: Any) -> CoordinatorOpT:
        return op(c=self, *args, **kwargs)
...
class BackupOp(SteppedCoordinatorOp):
    @staticmethod
    async def create(*, c: Coordinator) -> BackupOp:
        return BackupOp(c=c)
...
class DeltaBackupOp(SteppedCoordinatorOp):
    @staticmethod
    async def create(*, c: Coordinator) -> DeltaBackupOp:
...
class RestoreOp(SteppedCoordinatorOp):
    @staticmethod
    async def create(*, c: Coordinator, req: ipc.RestoreRequest = ipc.RestoreRequest()) -> RestoreOp:
